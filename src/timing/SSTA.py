...
def calculateCircuitDelay(
    ...
    if mosekStatus[0] >= 0:
        MaximumF = maxOfDistributionsMOSEK_UNARY(withSymmetryConstr)
    elif cvxpy and unary:
        MaximumF = maxOfDistributionsCVXPY_UNARY(withSymmetryConstr)
    elif cvxpy and not unary and not GP:
        MaximumF = maxOfDistributionsCVXPY_McCormick
    elif cvxpy and GP:
        MaximumF = maxOfDistributionsCVXPY_GP
    elif not cvxpy and unary and not mosekTRI:
        MaximumF = maxOfDistributionsUNARY
    elif monteCarlo:
        MaximumF = maxMonteCarlo
    else:
        MaximumF = maxOfDistributionsFORM
    if mosekStatus[0] >= 0:
        ConvolutionF = Convolution_UNARY_MOSEK(withSymmetryConstr)
    elif cvxpy and unary:
        ConvolutionF = Convolution_UNARY(withSymmetryConstr)
    elif cvxpy and not unary and not GP:
        ConvolutionF = Convolution_McCormick
    elif cvxpy and GP:
        ConvolutionF = Convolution_GP
    elif not cvxpy and unary:
        ConvolutionF = RandomVariable.convolutionOfTwoVarsNaiveSAME_UNARY
    elif monteCarlo:
        ConvolutionF = np.add
    else:
        ConvolutionF = RandomVariable.convolutionOfTwoVarsShift
    ...
    while not queue.empty():
        ...
        if tmpNode.prevDelays:  # calculate maximum + convolution - depends on the tool
            ...
            if usingMosek and not mosekTRI:
                ...
                maxDelay, curNofVariables, curNofConstr = MaximumF(
                    tmpNode.prevDelays, curNofVariables, curNofConstr
                )
                currentRandVar, curNofVariables, curNofConstr = ConvolutionF(
                    currentRandVar, maxDelay, curNofVariables, curNofConstr
                )
                ...
                currentRandVar, AllConstr = ConvolutionF(
                    currentRandVar, maxDelay, AllConstr
                )
    ...
    if len(sink) != 1:
        ...
        if usingMosek:
            sinkDelay, curNofVariables, curNofConstr = MaximumF(
                sink, curNofVariables, curNofConstr
            )
...
def Convolution_UNARY_MOSEK(withSymmetryConstr=False):
    ...
    def convolutionF(
        x1: RandomVariableMOSEK, x2: RandomVariableMOSEK, curNofVariables, curNofConstr
    ):
        return x1.convolution_UNARY_MAX_DIVIDE_VECTORIZED(
            x2, curNofVariables, curNofConstr, withSymmetryConstr=withSymmetryConstr
        )
...
def Convolution_UNARY(withSymmetryConstr=False):
    ...
    def convolutionF(
        x1: cvxpyVariable.RandomVariableCVXPY, x2: cvxpyVariable.RandomVariableCVXPY
    ):
        return x1.convolution_UNARY_DIVIDE(
            x2, withSepConstr=withSymmetryConstr, asMin=False
        )

    return convolutionF


def Convolution_McCormick(
    x1: cvxpyVariable.RandomVariableCVXPY, x2: cvxpyVariable.RandomVariableCVXPY
) -> cvxpyVariable.RandomVariableCVXPY:
    """
    Calculates convolution of an array of PDFs of cvxpy variable, is for clean code sake.
...
def Convolution_GP(
    x1: cvxpyVariable.RandomVariableCVXPY, x2: cvxpyVariable.RandomVariableCVXPY, constr
) -> (cvxpyVariable.RandomVariableCVXPY, []):
    """
    Calculates convolution of an array of PDFs of cvxpy variable, is for clean code sake.
...
def maxOfDistributionsMOSEK_UNARY(withSymmetryConstr=False) -> cp.Variable:
    ...
    def maximumF(delays: [RandomVariableMOSEK], curNofVariables, curNofConstr):
        ...
        for i in range(0, size - 1):
            newRV, newNofVariables, newNofConstr = delays[
                i
            ].maximum_UNARY_MAX_DIVIDE_VECTORIZED(
                delays[i + 1],
                curNofVariables,
                curNofConstr,
...
def maxOfDistributionsCVXPY_UNARY(withSymmetryConstr=False) -> cp.Variable:
    ...
    def maximumF(delays: [cvxpyVariable.RandomVariableCVXPY]):
        size = len(delays)
        for i in range(0, size - 1):
            newRV = delays[i].maximum_QUAD_UNARY_DIVIDE(
                delays[i + 1], withSymmetryConstr=withSymmetryConstr, asMin=False
            )
            delays[i + 1] = newRV
...
def maxOfDistributionsCVXPY_McCormick(
    delays: [cvxpyVariable.RandomVariableCVXPY],
) -> cp.Variable:
    """
    Calculates maximum of an array of PDFs of cvxpy variable
...
def maxOfDistributionsCVXPY_GP(
    delays: [cvxpyVariable.RandomVariableCVXPY], constr
) -> (cp.Variable, []):
    """
    Calculates maximum of an array of PDFs of cvxpy variable
